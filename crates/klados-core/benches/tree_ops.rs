...
fn load_instance(path: &str) -> Option<(Tree, IndexedBinTree, u32)> {
    ...
    let header_line = lines.find(|l| l.starts_with("#p"))?;
...
fn bench_clone_scaling(c: &mut Criterion) {
    ...
    let instances = vec![
        (
            "10_leaves",
            "../../../stride-downloads/00/00/926dfbc847ee6009bf51ada7bc2b",
        ),
        (
            "57_leaves",
            "../../../stride-downloads/02/e2/943b0192cc7cca00badd6265b93d",
        ),
        (
            "100_leaves",
            "../../../stride-downloads/03/02/e3b8c284b4b99c20c7136c6edc0e",
        ),
        (
            "192_leaves",
            "../../../stride-downloads/04/36/a0f3cbe3085c7ae4dc6ed05a74e9",
        ),
    ];
    ...
    for (name, path) in instances {
        if let Some((arena_tree, pointer_tree, num_leaves)) = load_instance(path) {
            ...
            println!(
                "\nLoaded {}: {} leaves, {} nodes",
                name, num_leaves, num_nodes
            );

            group.bench_with_input(BenchmarkId::new("arena", name), &num_nodes, |b, _| {
                b.iter(|| {
            ...
            group.bench_with_input(BenchmarkId::new("pointer", name), &num_nodes, |b, _| {
                b.iter(|| {
