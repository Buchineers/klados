...
fn singleton_absorption_pass(
    ...
    let mut lca1_by_root: HashMap<u32, NodeId> = HashMap::with_capacity(roots.len());
    let mut lca2_by_root: HashMap<u32, NodeId> = HashMap::with_capacity(roots.len());
    for &r in &roots {
        lca1_by_root.insert(r, component_lca(tp1, &comp_labels[r as usize]));
        lca2_by_root.insert(r, component_lca(tp2, &comp_labels[r as usize]));
    }

    let mut singleton_roots: Vec<u32> = roots
        .iter()
        .copied()
        .filter(|&r| comp_labels[r as usize].len() == 1)
        .collect();
    singleton_roots.sort_unstable();
    ...
    for s in singleton_roots {
        ...
        let rs = uf.find(s);
        if comp_labels[rs as usize].len() != 1 {
            continue;
        }

        let Some(ls1) = lca1_by_root.get(&rs).copied() else {
            continue;
        };
        let Some(ls2) = lca2_by_root.get(&rs).copied() else {
            continue;
        };

        let mut candidates: Vec<(CandidateKey, u32)> = Vec::new();
        for &r in &roots {
            if r == rs {
                continue;
            }
            let rr = uf.find(r);
            if rr == rs || comp_labels[rr as usize].is_empty() {
                continue;
            }
            let Some(lr1) = lca1_by_root.get(&rr).copied() else {
                continue;
            };
            let Some(lr2) = lca2_by_root.get(&rr).copied() else {
                continue;
            };

            let merged_lca1 = tp1.lca(ls1, lr1);
            let merged_lca2 = tp2.lca(ls2, lr2);
            let jump1 = if merged_lca1 != NONE {
                (tp1.depth[ls1 as usize] + tp1.depth[lr1 as usize])
                    .saturating_sub(2 * tp1.depth[merged_lca1 as usize])
            } else {
                u16::MAX
            };
            let jump2 = if merged_lca2 != NONE {
                (tp2.depth[ls2 as usize] + tp2.depth[lr2 as usize])
                    .saturating_sub(2 * tp2.depth[merged_lca2 as usize])
            } else {
                u16::MAX
            };
            let sz = comp_labels[rr as usize].len() as u16;
            candidates.push((
                CandidateKey {
                    lca_jump_sum: jump1.saturating_add(jump2),
                    tree_hits_rank: 0,
                    rise_sum: 0,
                    balance_penalty: sz.saturating_sub(1),
                    size_sum: sz.saturating_add(1),
                    tie_a: rs.min(rr),
                    tie_b: rs.max(rr),
                },
                rr,
        ...
        candidates.sort_unstable_by_key(|&(k, r)| (k, r));
        for &(_, target) in &candidates {
            if terminate.load(Ordering::Relaxed) || started.elapsed() >= time_limit {
                break;
            }

            let rt = uf.find(target);
            let rs_now = uf.find(rs);
            if rt == rs_now || comp_labels[rt as usize].is_empty() || comp_labels[rs_now as usize].len() != 1 {
                continue;
            }
...
fn local_repair_pass(
    ...
    loop {
        ...
        let max_repair_size = roots.len().max(2);
        let max_repair_eval = roots.len().saturating_mul(roots.len().saturating_sub(1)) / 2;

        let local_max_depth = tp1
            .depth
            .iter()
        ...
        let mut round_merges = 0usize;
        for &r in &roots {
            let rr = uf.find(r);
            if rr != r || comp_labels[rr as usize].is_empty() {
                continue;
            }
            let sz = comp_labels[rr as usize].len();
            if sz > max_repair_size {
                continue;
            }
            let neighbors = collect_nearby_roots(rr, &leaf_to_root, comp_labels, tp1, tp2, local_max_depth);
            for (other, _, _) in neighbors {
                if terminate.load(Ordering::Relaxed) || started.elapsed() >= time_limit {
                    break;
                }
                if evals >= max_repair_eval {
                    break;
                }
                evals += 1;
                ...
                let osz = comp_labels[ro as usize].len();
                if osz > max_repair_size {
                    continue;
                }
                if try_merge(
                    rr,
