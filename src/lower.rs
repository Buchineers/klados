...
pub fn solve_lower(instance: &Instance) -> Option<Vec<Tree>> {
    ...
    {
        let mut lagr = klados_heuristic::lagrangian::LagrangianSolver::new();
        ...
        let lb = klados_heuristic::HeuristicSolver::stats(&lagr).lower_bound;
    ...
    if instance.num_leaves <= BP_MAX_LEAVES && Instant::now() < deadline {
        ...
        if let Some(forest) = klados_exact::bp::bp_solve_capped(instance, &terminate) {
