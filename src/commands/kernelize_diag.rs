...
pub fn run(instance: &Instance) -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!(
        "After kernelization: {} leaves, param_reduction={} ({:.1}% removed, {:.2}ms)",
        n_kern,
        param_red,
    ...
    eprintln!(
        "Optimal MAF: {} components (solved in {:.2}ms)",
        maf_size,
    ...
    eprintln!(
        "Singletons in optimal MAF: {} leaves: {:?}",
    ...
    eprintln!(
        "Singletons detected by 3-2 rule: {} {:?}",
        already_found.len(),
        already_found
    );
    eprintln!(
        "Singletons MISSED by kernelization: {} {:?}",
        missed.len(),
        missed
    );
    ...
    if missed.is_empty() {
            ...
            for (t_idx, tree) in instance.trees.iter().enumerate() {
                ...
                let sib_info = match sib {
                    Some(s) if tree.is_leaf(s) => {
                        format!("leaf sibling={}", tree.label[s as usize])
                    }
        ...
        eprintln!();
        eprintln!(
            "Potential kernel improvement: {} → {} leaves ({} more leaves removable)",
            n_kern,
